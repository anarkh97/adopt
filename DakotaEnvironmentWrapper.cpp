...
DakotaEnvironmentWrapper::DakotaEnvironmentWrapper(
  ProgramOptions options, bool check_bcast_construct,
...
DakotaEnvironmentWrapper::DakotaEnvironmentWrapper(
  MPI_Comm dakota_comm, ProgramOptions options, bool check_bcast_construct,
  DbCallbackFunctionPtr callback, void *callback_data)
...
void DakotaEnvironmentWrapper::SetupAdaptiveOptimizationVariables()
{
  ...
  if (world_rank == 0)
  {
    ...
    size_t num_interp_points
      = problem_db.get_sizet("variables.discrete_state_range");
    size_t max_func_evals
      = problem_db.get_sizet("method.max_function_evaluations");
    ...
    problem_db.set("variables.discrete_state_range.labels", updated_labels);
    problem_db.set("variables.discrete_state_range.categorical",
                   updated_catgry);
    problem_db.set("variables.discrete_state_range.initial_state",
                   initial_state);
    problem_db.set("variables.discrete_state_range.lower_bounds", lower_bounds);
    problem_db.set("variables.discrete_state_range.upper_bounds", upper_bounds);
    ...
    const String &tr_work_dir = problem_db.get_string("interface.workDir");
