...
void DakotaEnvironmentWrapper::SetupAdaptiveOptimizer()
{
  ...
  if(adaptive_optimization) {
    ...
    std::shared_ptr<Model> sim_model = topLevelIterator->iterated_model();
    sim_model.reset(new SimulationModel(problem_db)); /* un-scaled copy */
    sim_model->inactive_view(MIXED_STATE);
