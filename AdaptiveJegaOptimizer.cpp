...
AdaptiveJegaOptimizer::AdaptiveJegaOptimizer(ProblemDescDB    &prob_db,
    ...
    : Optimizer(prob_db, sim_model,
                std::shared_ptr<AdaptiveJegaTraits>(new AdaptiveJegaTraits())),
      param_db(nullptr), eval_creator(nullptr), decision_maker(nullptr)
{
  ...
  // by DakotaOptimizer
  eval_creator = make_shared<JegaEvaluatorCreator>(*iteratedModel, *err_model,
                                                   *decision_maker);
}
...
void AdaptiveJegaOptimizer::core_run()
{
  EDDY_FUNC_DEBUGSCOPE
  ...
  while (driver.PerformNextIteration(ga_algorithm))
  {
    ...
    //    this,
    //    text_entry(lfatal(), name
    //                           + ": Re-evaluations of best designs requested "
    //                             "and failed.\n"))
    //}
  ...
  JEGA_LOGGING_IF_ON(double elapsed
                     = static_cast<double>((clock() - start)) / CLOCKS_PER_SEC;)

  JEGALOG_II(
    ga_algorithm->GetLogger(), lquiet(), this,
    ostream_entry(lquiet(), "JEGA Front End: " + ga_algorithm->GetName()
                              + " execution took ")
      << elapsed << " seconds.")
...
void AdaptiveJegaOptimizer::LoadAlgorithmConfig(AlgorithmConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void AdaptiveJegaOptimizer::LoadProblemConfig(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void AdaptiveJegaOptimizer::LoadDesignVariables(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void AdaptiveJegaOptimizer::LoadObjectiveFunctions(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void AdaptiveJegaOptimizer::LoadConstraints(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void AdaptiveJegaOptimizer::GetBestSOSolutions(
...
void AdaptiveJegaOptimizer::LoadDakotaResponses(const Design &from,
                                                Variables    &vars,
                                                Response     &resp) const
{
...
//void AdaptiveJegaOptimizer::ExecuteErrorCalculations(GeneticAlgorithm &the_ga)
//{
...
//bool AdaptiveJegaOptimizer::CorrectBestDesigns(GeneticAlgorithm &the_ga)
//{
//
//  const string &name = the_ga.GetName();
//
