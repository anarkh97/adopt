      ...
      param_db(nullptr), eval_creator(nullptr), decision_maker(nullptr)
{
  ...
  //! Exit if MultiFidelityGAOptimizer was called with moga.
  if (methodName == Dakota::MOGA)
  {
    JEGALOG_II_G(lfatal(), this,
                 text_entry(lfatal(), "Adaptive JEGA Error: Multi-objective "
  ...
  if (!Driver::IsJEGAInitialized())
  {
    ...
    Driver::InitializeJEGA("JEGAGlobal.log", jega_level, rand_seed,
                           Logger::ABORT, jega_register_signals);
  }

  // exit if initialization failed
  JEGAIFLOG_II_G_F(
    !Driver::IsJEGAInitialized(), this,
  ...
  // by DakotaOptimizer
  eval_creator = make_shared<MFGAEvaluatorCreator>(*iteratedModel, *err_model,
                                                   *decision_maker);
}
...
void MultiFidelityGAOptimizer::core_run()
{
  EDDY_FUNC_DEBUGSCOPE
  ...
  // Create a new driver for MultiFidelityGAOptimizer.
  MFGADriver driver(prob_config);
  ...
  if (!this->_initPts.empty())
  {
    ...
    GeneticAlgorithmInitializer *newInit
      = AllOperators::FullInstance().GetInitializer("double_matrix",
                                                    *ga_algorithm);
    ...
    JEGAIFLOG_II_F(
      !ga_algorithm->SetInitializer(newInit), ga_algorithm->GetLogger(), this,
      text_entry(lfatal(),
                 name
                   + ": Unable to set the initializer to "
                     "double_matrix because it is incompatible with the other "
                     "operators."))

    JEGAIFLOG_II_F(
      !newInit->ExtractParameters(pdb), ga_algorithm->GetLogger(), this,
      text_entry(lfatal(), name + ": Failed to retrieve the parameters for \""
                             + newInit->GetName() + "\"."));
  }
  ...
  JEGAIFLOG_II_G(
    design_sort_map.size() == 0, lquiet(), this,
    text_entry(
      lquiet(),
      name
        + ": was unable to identify at least one "
...
JEGA::DoubleMatrix
MultiFidelityGAOptimizer::ToDoubleMatrix(const VariablesArray &variables) const
{
  EDDY_FUNC_DEBUGSCOPE
...
void MultiFidelityGAOptimizer::LoadParameterDatabase()
{
  EDDY_FUNC_DEBUGSCOPE
  ...
  if (param_db)
    param_db.reset();
  param_db
    = shared_ptr<BasicParameterDatabaseImpl>(new BasicParameterDatabaseImpl());
  ...
  const Real &constraint_penalty
    = prob_db.get_real("method.constraint_penalty");
  ...
  if (prob_db.get_string("method.mutation_type") != "")
    param_db->AddDoubleParam("method.mutation_rate",
                             prob_db.get_real("method.mutation_rate"));
  else
    param_db->AddDoubleParam("method.mutation_rate", 0.1);

  param_db->AddDoubleParam("method.mutation_scale",
                           prob_db.get_real("method.mutation_scale"));
  ...
  param_db->AddSizeTypeParam(
    ...
    (size_t)(prob_db.get_sizet("method.max_function_evaluations")));
  ...
  const string &crossover_operator
    = prob_db.get_string("method.crossover_type");
  ...
  param_db->AddIntegralParam("method.output",
                             prob_db.get_short("method.output"));
  ...
  const string &convergence_operator
    = prob_db.get_string("method.jega.convergence_type");

  if (convergence_operator != "")
    param_db->AddStringParam("method.jega.convergence_type",
  ...
  param_db->AddDoubleVectorParam("method.jega.niche_vector", niche_vector);

  dak_rv = &prob_db.get_rv("method.jega.distance_vector");
  JEGA::DoubleVector distance_vector(dak_rv->values(),
                                     dak_rv->values() + dak_rv->length());

  param_db->AddDoubleVectorParam("method.jega.distance_vector",
                                 distance_vector);
...
void MultiFidelityGAOptimizer::LoadAlgorithmConfig(AlgorithmConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
  ...
  config.SetAlgorithmName(
    method_id().empty() ? method_enum_to_string(methodName) : method_id());
}

//-----------------------------------------------------------------------------

void MultiFidelityGAOptimizer::LoadProblemConfig(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void MultiFidelityGAOptimizer::LoadDesignVariables(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void MultiFidelityGAOptimizer::LoadObjectiveFunctions(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
...
void MultiFidelityGAOptimizer::LoadConstraints(ProblemConfig &config)
{
  EDDY_FUNC_DEBUGSCOPE
  ...
  **************************************************************************/
  ...
  for (size_t i = 0; i < numLinearIneqConstraints; ++i)
  {
    ...
    config.AddLinearTwoSidedInequalityConstraint(
...
void MultiFidelityGAOptimizer::GetBestSOSolutions(
  const DesignOFSortSet &from, const GeneticAlgorithm &ga,
...
//void MultiFidelityGAOptimizer::ExecuteErrorCalculations(GeneticAlgorithm &the_ga)
//{
//  shared_ptr<MFGAEvaluator> evaluator(0x0);
//  try
//  {
//    GeneticAlgorithmEvaluator &base_eval
//      = the_ga.GetOperatorSet().GetEvaluator();
//    MFGAEvaluator &sub_eval = dynamic_cast<MFGAEvaluator &>(base_eval);
//    evaluator               = std::shared_ptr<MFGAEvaluator>(
//      &sub_eval,
//      [](MFGAEvaluator *) { /* do not delete the orignal evaluator */ });
//  }
...
//      this,
//      text_entry(lfatal(),
//                 name + ": Expected MFGAEvaluator as the GA evaluator.\n"))
//  }
...
//bool MultiFidelityGAOptimizer::CorrectBestDesigns(GeneticAlgorithm &the_ga)
//{
//
//  const string &name = the_ga.GetName();
//
